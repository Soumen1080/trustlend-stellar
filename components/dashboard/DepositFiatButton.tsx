...
interface DepositFiatButtonProps {
  /** User wallet address (G...). If null the button prompts to connect. */
...
const STEP_LABEL: Record<Step, string> = {
  ...
  initiating: "Starting deposit…",
  interactive: "Complete the steps in the anchor window",
  done: "",
  error: "",
};

export function DepositFiatButton({ walletAddress }: DepositFiatButtonProps) {
  ...
  useEffect(() => {
    if (!open) {
      const trigger = document.querySelector('[data-deposit-trigger]') as HTMLElement;
      trigger?.focus();
    } else {
  ...
  const startDeposit = useCallback(async () => {
    ...
    try {
      ...
      // 4. Start the interactive deposit → get the anchor's hosted URL.
      setStep("initiating");
      const session = await startInteractiveDeposit(endpoints, jwt, {
        account,
        assetCode: config.assetCode,
        amount: amount.trim() || undefined,
      });

      // 5. Open the interactive URL in a popup for the user to provide fiat
      //    payment details. Fall back to an inline link if it's blocked.
      setInteractiveUrl(session.url);
      const popup = window.open(
        session.url,
        "trustlend_sep24_deposit",
      ...
      <article className="workspace-card workspace-card--full">
        ...
        >
          <div>
            ...
            >
              Deposit fiat currency and receive {config.assetCode} in your wallet via a
        ...
        >
          <FocusTrap active={!busy} initialFocusRef={step === "idle" ? startButtonRef : closeButtonRef}>
            ...
            >
              ...
              >
                <h3 id="deposit-modal-title" style={{ margin: 0, fontSize: "1.1rem", fontWeight: 800 }}>
              ...
              {step === "error" && (
                <div style={{ marginTop: "1.25rem" }}>
                  ...
                  <button type="button" onClick={() => void startDeposit()} style={primaryBtnStyle}>
                    Try Again
                  </button>
                </div>
              )}
